...
import Login from './Containers/Login/Login'; // adjust the path if needed
...
//   let surname = location.pathname.split("/")[2];
...
// }
useEffect(() => {
  if (location.pathname !== "/" && location.pathname !== "/browse" && location.pathname !== "/login" ) {
    const surname = location.pathname.split("/")[2];
...
}, [location.pathname, games]);  // important: include both dependencies

useEffect(() => {
  if (currentUser === false) {
    navigate('/login');
  }
}, [currentUser]);  // important: include both dependencies
